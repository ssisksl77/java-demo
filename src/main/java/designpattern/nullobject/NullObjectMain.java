 ...
 *  이러한 접근의 장점은 객체의 부재를 디폴트 구현으로 넘겨 예상가능하게(그냥 null이 넘어오는 것보다) 하며 사이트이팩트가 없다.(하는일이 없으니까)
 *  널객체를 리턴하면서(예, 빈 리스트), 값이 null인지 아닌지 뿐만아니라, 실제 리스트인지 아닌지 확인할 필요도 없다. 그냥 쓰던데로 포문을 돌리거나 map을 쓰면 된다. (그러면 저절로 아무것도 안할 것이다)
